...
func (s *Client) GetASREvaluationStatusV1(skillId string, evaluationId string) (response *evaluations_.GetAsrEvaluationStatusResponseObject, err error) {
	h := swaggerlt.NewRequestHelper("get", s.Endpoint, "/v1/skills/{skillId}/asrEvaluations/{evaluationId}/status")
	h.Path("skillId", skillId)
	h.Path("evaluationId", evaluationId)
	response = &evaluations_.GetAsrEvaluationStatusResponseObject{}
	h.Response = response
